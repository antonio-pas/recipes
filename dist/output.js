(() => {
  ...
  typeof window != "undefined" && window.__PREACT_DEVTOOLS__ && window.__PREACT_DEVTOOLS__.attachPreact("10.7.1", l, { Fragment: d, Component: _ });

  // app.jsx
  var App = () => {
    const [data, setData] = m2();
    const [loading, setLoading] = m2(true);
    const [searchQuery, setSearchQuery] = m2("");
    y2(() => {
      fetch("https://www.themealdb.com/api/json/v1/1/search.php?s=").then((res) => res.json()).then((json) => {
        setLoading(false);
        setData(json);
      });
    }, []);
    const search = (e3) => {
      e3.preventDefault();
      setLoading(true);
      fetch(`https://www.themealdb.com/api/json/v1/1/search.php?s=${searchQuery.split(" ").join("_")}`).then((res) => res.json()).then((json) => {
        setLoading(false);
        setData(json);
      });
    };
    return /* @__PURE__ */ v("div", {
      class: "bg-zinc-900 flex flex-col gap-6 p-6 min-h-screen text-white"
    }, /* @__PURE__ */ v("form", {
      class: "flex max-h-10 gap-6 max-w-xs"
    }, /* @__PURE__ */ v("input", {
      class: "bg-zinc-800 focus:bg-zinc-700 p-2 placeholder:text-zinc-400 rounded-lg grow focus:outline-1 focus:outline-zinc-200",
      type: "text",
      onInput: (e3) => {
        setSearchQuery(e3.target.value);
      },
      value: searchQuery,
      ...
      return /* @__PURE__ */ v("div", {
        class: "flex flex-col sm:rounded-lg p-4 bg-zinc-800"
      }, /* @__PURE__ */ v("img", {
        class: "rounded-lg max-w-full object-contain",
        src: el.strMealThumb
      }), /* @__PURE__ */ v("h1", {
        class: "text-3xl font-black text-center"
      }, el.strMeal), /* @__PURE__ */ v("h2", {
        class: "text-xl text-center"
      }, el.strArea), /* @__PURE__ */ v("ul", {
        class: "ml-6 mb-4 list-disc"
      }, Object.keys(el).map((x3) => {
        if (x3.startsWith("strIngredient") && el[x3] != "" && el[x3] != null) {
          let measure = `strMeasure${x3.split("t")[2]}`;
          return /* @__PURE__ */ v("li", null, el[measure], " ", el[x3]);
        }
      })), /* @__PURE__ */ v("p", {
